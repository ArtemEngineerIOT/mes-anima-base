...
import type { ReleaseProductionEventsSummarySnapshot } from "./types";
...
function parseSummaryRecord(payload: unknown): Record<string, unknown> | null {
    ...
    if (rawChangedAtString && hasCompositePairs) {
        ...
        return {
            ...
            changed_at: pairs.changed_at || rawChangedAtString,
        };
    }
...
export function mapReleaseProductionEventsSummaryChangedPayload(
    payload: unknown,
    ...
    return buildReleaseProductionEventsSummarySnapshot(record);
}

export function readReleaseProductionEventsSummaryChangedWorkAreaId(
    payload: unknown,
): string | undefined {
    const record = parseSummaryRecord(payload);
    if (!record) {
