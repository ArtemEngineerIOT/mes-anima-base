...
import type { DefectWeighingStage } from "./types";
...
function formatQuantity(quantity: number | undefined, unit: string | undefined): string {
    ...
    const formatted = new Intl.NumberFormat("ru-RU", { maximumFractionDigits: 2 }).format(quantity);
    return unit ? `${formatted} ${unit}` : formatted;
}

function mapStageRow(row: Record<string, unknown>): DefectWeighingStage | null {
...
export function mapDefectWeighingPlanStagesPayload(
    payload: ApiSchemas["MaterialOrderPlanStagesResponse"] | undefined,
): DefectWeighingStage[] {
    const fallbackMessage = "Не удалось загрузить этапы";
    ...
    return (wrapper.result ?? [])
        ...
        .filter((row): row is DefectWeighingStage => row !== null);
}
